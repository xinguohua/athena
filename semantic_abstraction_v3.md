...
**结构化解析。** 对 $D_i$ 逐句执行 spaCy 依存分析，按以下规则提取和过滤三元组：

*提取。* 遍历句中词性为动词（`pos_ == VERB`）且依存标签非助动词（`dep_ != aux`）的节点作为谓语候选。对每个谓语，沿依存弧向上收集名词性主语（`nsubj`、`nsubjpass`），向下收集直接宾语（`dobj`）、介词宾语（`prep → pobj`）和属性补语（`attr`、`oprd`），组合为候选三元组。例如 "Adversaries may inject DLLs into processes" 中，谓语节点为 inject，向上收集主语 Adversaries，向下收集宾语 DLLs 和介宾 processes，提取出 ⟨Adversaries, inject, DLLs into processes⟩。

*过滤。* 三类句子被丢弃：(1) 谓语为系动词（be）的定义句——如 "DLL injection is a method of executing arbitrary code"，这类句子描述概念而非攻击动作；(2) 缺少名词性主语的句子——通常为检测建议或被动描述；(3) 主语为模糊代词（This、They、It）的句子——将其回溯替换为前句中的名词性先行词，若无法回溯则丢弃。

*谓语规范化。* 通用包装动词（use、abuse、leverage 等）不携带操作语义，被替换为谓语短语中内嵌的真正动作动词。替换规则：从谓语短语中匹配 "to + 动词" 模式（如 "use tools to **dump**" → dump，"abuse BITS to **establish**" → establish）；若谓语短语无内嵌动词，则从宾语上下文推断实际动作。

*人工校验。* 自动提取后对全部 691 项技术逐一校验，检查三项：(1) 攻击行为性——三元组须描述攻击者的主动行为，而非背景定义；(2) 主语具体性——主语须为明确实体（如 Adversaries、工具名），不得为抽象短语；(3) 工具覆盖性——原文提及的具体工具、命令、API 须在三元组中保留。不合格者修正后重新纳入。
