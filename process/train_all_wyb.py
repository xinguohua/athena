...
DATASET_NAME = "optcday1"          # 可切换数据集 (基础数据集名)
...
GLOBAL_ID = "wyb-prographper-optcday1-0660"  # 按需修改，例如 "B"、"alice"、"ci-001"
...
if __name__ == "__main__":
    ...
    print("\n===== Training Statistics =====")
    if hours > 0:
        print(f"Total Training Time: {hours}h {minutes}m {seconds:.2f}s ({wall:.2f}s)")
    elif minutes > 0:
        print(f"Total Training Time: {minutes}m {seconds:.2f}s ({wall:.2f}s)")
    else:
        print(f"Total Training Time: {seconds:.2f}s")
