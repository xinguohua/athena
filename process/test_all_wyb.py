...
GLOBAL_ID = "wyb-prographper-optcday1-0660"
...
def save_snapshot_nodes(all_snapshots, output_file: Path = None) -> Optional[Path]:
    ...
    # 检查是否为 OPTC 数据集（通过 GLOBAL_ID 判断）
    is_optc = "optc" in str(GLOBAL_ID).lower() if GLOBAL_ID else False
    ...
    try:
        with output_file.open("w", encoding="utf-8") as f:
            ...
            for i, snapshot in enumerate(all_snapshots):
                ...
                # OPTC 数据集特殊处理：node_type 可能是字符串和整数混合
                if is_optc:
                    # 将所有 node_type 转为字符串后排序
                    for t, c in sorted(node_type_count.items(), key=lambda x: str(x[0])):
                        f.write(f"      {t}: {c}个\n")
                else:
                    # 其他数据集使用原始排序（保持兼容性）
